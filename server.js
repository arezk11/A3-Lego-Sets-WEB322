...
app.get("/about", (req, res) => {
  res.render("about"); // Assuming "about.ejs" is in the "views" directory
});
...
app.get("/lego/sets", (req, res) => {
  ...
  if (theme) {
    legoData.getSetsByTheme(theme)
      .then(data => {
        res.render("sets", { sets: data }); // Render the "sets" view with legoSets data
      })
      .catch(error => {
        res.status(404).send(error);
      });
...
app.get("/lego/sets/:set_num", (req, res) => {
  ...
  legoData.getSetByNum(setNum)
    .then(data => {
      res.render("set", { set: data }); // Render the "set" view with legoSet data
    })
    .catch(error => {
      res.status(404).send(error);
    });
